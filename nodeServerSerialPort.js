...
function formatWaypointMsg(waypointMsg) {
    waypointArray = waypointMsg.split(" ");
    ...
    console.log("the last element in the array is " + waypointArray[waypointArray.length-1]);
    numPoints = waypointArray.length/2;
...
function messageSenderSM() {
    ...
    function sendMessage() {
        if (this.seq >= (numPoints*5)+1){
            createWaypointMessage(this.seq, 16, 0, 0, points[0][0], points[0][1], 5);
            this.seq++;
            return;
        }
        if (this.curnt == -1) {
...
function createMissionCountMessage() {
    mav.createMessage("MISSION_COUNT", {
        ...
        console.log("Count was " + ((numPoints*5)+1));
        // console.log("numPoints was " + numPoints);
        // console.log("numberPoints was " + numberPoints);
    });
};

function createWaypointMessage(sequence, cmd, param1, param2, x, y, z) {
        console.log("PARAM 1 IS .... " + param1);
        console.log(sequence + ", " + cmd + ", " + x + ", " + y);
