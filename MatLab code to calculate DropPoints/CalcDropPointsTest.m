...
addpath 'C:\Users\dsamson\Documents\ServerDroneCode\DroneWeb\MatLab code to calculate DropPoints\Domain Optimization\';
pyname = 'Sorting_Methods_Reduced_SERVER_Implement.py';

pos = [pos; pos(1,:)];

pos_xy = pi/180*(pos-repmat([pos(1,1),pos(1,2)],size(pos,1),1))*earthRadius;
...
%pos2 = pos_xy - vecN*radius/sqrt(3);
...
%close(gcf);
p_deg= 180/pi*(p)/earthRadius +repmat([pos(1,1),pos(1,2)],size(p,1),1);
...
length(waypoints)
figure
r_deg = radius*180/pi/earthRadius;
viscircles(waypoints, repmat(r_deg, size(waypoints,1),1), 'LineWidth', 1);